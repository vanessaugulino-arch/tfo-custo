...
/** Painel de admin temporário — gera nova senha temporária para um usuário existente. Restrito às sócias. */
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { error: erroReset } = await adminClient.auth.admin.updateUserById(userId, {
      password: senha,
      user_metadata: { senha_temporaria: true },
    });

    if (erroReset) {
      return new Response(JSON.stringify({ error: erroReset.message }), {
        status: 400,
        headers: { ...CORS_HEADERS, "Content-Type": "application/json" },
      });
    }
